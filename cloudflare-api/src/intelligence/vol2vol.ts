...
export type Vol2VolConfidence = 'HIGH' | 'MEDIUM' | 'LOW'
...
export function analyzeVol2Vol(params: {
  ...
  const priceDirection = direction(priceChange)
  const volumeDirection = direction(volumeChange)
  const oiDirection = direction(openInterestChange)
  ...
  if (positioning === 'LONG_BUILDUP') score += 15
  if (positioning === 'SHORT_BUILDUP') score -= 15
  if (positioning === 'SHORT_COVERING') score += 5
  if (positioning === 'LONG_LIQUIDATION') score -= 5

  score = clamp(score, -100, 100)

  const reasons: string[] = []

  if (priceDirection === 'UP') reasons.push('Price is rising')
  if (priceDirection === 'DOWN') reasons.push('Price is falling')
  ...
  if (positioning !== 'NEUTRAL') {
    reasons.push(`Positioning: ${positioning}`)
  }
  ...
  if (positioning === 'SHORT_COVERING' && score < 50) {
    signal = 'LONG_EXIT'
  }

  if (positioning === 'LONG_LIQUIDATION' && score > -50) {
    signal = 'SHORT_EXIT'
  }

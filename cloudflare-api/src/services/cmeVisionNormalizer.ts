...
const putVolume = finiteNumber(
  ...
  // ------------------------------------------------------------
  // Derive option series from readable option rows when Vision
  // does not explicitly return option_series.
  // ------------------------------------------------------------
  if (optionSeries.length === 0 && optionRows.length > 0) {

...
import { getCachedMarketQuote } from '../services/market/quoteCache.js'
...
try {
  ...
  return await getCachedMarketQuote(market.provider ?? "mock", normalizedSymbol, marketProvider)
} catch (error) {
  const message =
  ...
  app.get("/api/market/intelligence", async (request, reply) => {
    ...
    try {
      ...
      if (!Number.isInteger(outputsize) || outputsize < 2) {
        ...
        const cached = await getCachedMarketIntelligence(
          cacheKey,
          async () => {
            const candles = await getHistory.call(provider, symbol, {
              interval,
              outputsize
            })
  ...
  app.get('/api/market/relationship', async (request, reply) => {
    ...
    try {
      ...
      const [baseQuote, quoteQuote] = await Promise.all([
        getCachedMarketQuote(baseMarket.provider ?? 'mock', baseSymbol, baseProvider),
        getCachedMarketQuote(quoteMarket.provider ?? 'mock', quoteSymbol, quoteProvider)
      ])

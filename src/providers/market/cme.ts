...
export class CmeMarketProvider implements MarketProvider {
  async getQuote(symbol: string): Promise<MarketQuote> {
    ...
    if (typeof price !== "number" || !Number.isFinite(price)) {
      const latest = chart.quotes?.at(-1)?.close;
  ...
  async getHistory(
    ...
    const outputsize = Math.min(Math.max(params.outputsize ?? 100, 1), 5000);
    ...
    const period2 = params.endDate ? new Date(params.endDate) : new Date();

...
export default function MarketPage() {
  ...
  async function loadMarket() {
    try {
      ...
      const [xauRes, gcRes, intelligenceRes] =
        await Promise.all([
          fetch(`${API}/api/market/quote?symbol=XAUUSD`),
          fetch(`${API}/api/market/quote?symbol=GC`),
          fetch(
            `${API}/api/market/intelligence?symbol=XAUUSD&interval=1h&outputsize=50`
          ),
        ])

      if (!xauRes.ok || !gcRes.ok || !intelligenceRes.ok) {
        throw new Error("Market API unavailable")
      }

      const [xauData, gcData, intelligenceData] =
        await Promise.all([
          xauRes.json(),
          gcRes.json(),
          ...
          <Card className="border-red-500/20 bg-red-500/5">
            <CardContent className="p-4">
              ...
              <p className="mt-1 text-xs text-zinc-500">
                Make sure iqtf-enterprise API is running on port 3000.
              </p>
            </CardContent>
          </Card>
        )}
